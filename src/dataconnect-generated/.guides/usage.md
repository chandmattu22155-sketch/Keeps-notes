...
```ts
import { useGetMyNotes, useCreateNote, useGetAllTags, useCreateTag } from '@dataconnect/generated/react';
// The types of these hooks are available in react/index.d.ts

const { data, isPending, isSuccess, isError, error } = useGetMyNotes();

const { data, isPending, isSuccess, isError, error } = useCreateNote(createNoteVars);

const { data, isPending, isSuccess, isError, error } = useGetAllTags();
...
```js
import { getMyNotes, createNote, getAllTags, createTag } from '@dataconnect/generated';


// Operation GetMyNotes: 
